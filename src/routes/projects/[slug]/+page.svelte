<script lang="ts">
    ...
    const filteredNotes = $derived(
        instance.currentNotes.filter(n => n.content.toLowerCase().includes(noteSearch.toLowerCase()))
    );
...
{#if project}
    <div class="flex-1 h-full flex flex-col gap-3 overflow-hidden">
        <ProjectHeader {project} bind:noteSearch={noteSearch} />
    ...
    <div class="w-80 h-full flex flex-col gap-3 shrink-0 overflow-hidden">
        <ProjectStats {project} notes={instance.currentNotes} />

        <ProjectScratchPad {project} />
    </div>
{:else}
    <div class="flex-1 h-full flex items-center justify-center text-sm text-gray-400 italic">
